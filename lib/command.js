...
function addOptionToCommand(command, option) {
    ...
    if (option.normalize === showCommandHelp) {
        option.defValue = undefined;
    }
...
var Command = function(name, params, config) {
    ...
    if ('defaultHelp' in config === false || config.defaultHelp) {
        this.infoOption('-h, --help', 'Output usage information', commands =>
            showCommandHelp(this, commands.slice(0, commands.length - 1).map(command => command.name))
        );
    }
    ...
    parse: function(args, suggest) {
        ...
        if (!Command.errorHandler) {
            ...
            try {
                ...
                Command.errorHandler(e.message || e);
            }
        }
    },
